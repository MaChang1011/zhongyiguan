			...
			<div class="row">
				<div class="col-sm-12">
					...
					<jsp:include page="../comm/operation.jsp">
						<jsp:param value="${ctx }/case/add" name="url"/>
					</jsp:include>
							...
							<tbody>
								<c:forEach items="${data.list }" var="entity" varStatus="status">
									<tr>
										...
										<td>${entity.title }</td>
										<td><c:choose>
											<c:when test="${entity.category==10 }">经典案例</c:when>
											<c:when test="${entity.category==20 }">心血之家</c:when>
										</c:choose></td>
										<td>${entity.pubuser }</td>
										<td><c:choose>
											<c:when test="${entity.type==1 }">医生</c:when>
											<c:when test="${entity.type==2 }">管理员</c:when>
										</c:choose></td>
										<td><c:choose>
										...
										<td>
											<a href="${ctx }/case/detail/${entity.id }" target="_blank" class="pdr5">详情</a>
											<a href="${ctx }/case/edit/${entity.id }" class="pdr5">修改</a>
											<c:choose>
					...
					<jsp:include page="../comm/pagin.jsp">
						<jsp:param value="case/index" name="url"/>
